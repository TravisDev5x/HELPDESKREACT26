...
import { Activity, Plus, Pencil, Trash2, ArrowLeft, Loader2 } from "lucide-react";
import { handleAuthError, getApiErrorMessage } from "@/lib/apiErrors";

const emptyForm = { name: "", description: "", is_active: true };

export default function EmployeeStatusesIndex() {
    const { can } = useAuth();
    ...
    const fetchItems = useCallback(async () => {
        setLoading(true);
        try {
            const { data } = await axios.get("/api/timedesk/employee-statuses");
            setItems(Array.isArray(data) ? data : []);
        } catch (err) {
            if (!handleAuthError(err)) {
                notify.error(getApiErrorMessage(err, "No se pudieron cargar los estatus"));
            }
        } finally {
            setLoading(false);
        }
        ...
        try {
            if (editing) {
                const { data } = await axios.put(
                    `/api/timedesk/employee-statuses/${editing.id}`,
                    form
                );
                setItems((prev) => prev.map((r) => (r.id === data.id ? data : r)));
                notify.success("Estatus actualizado");
            } else {
                const { data } = await axios.post("/api/timedesk/employee-statuses", form);
                setItems((prev) => [data, ...prev]);
                notify.success("Estatus creado");
            }
            setOpen(false);
            resetForm();
        } catch (err) {
    ...
    const toggleActive = async (item) => {
        ...
        try {
            const { data } = await axios.put(
                `/api/timedesk/employee-statuses/${item.id}`,
                { is_active: next }
            );
            setItems((prev) => prev.map((r) => (r.id === data.id ? data : r)));
            notify.success(next ? "Estatus activado" : "Estatus desactivado");
        } catch (err) {
            if (!handleAuthError(err)) {
    ...
    const handleDelete = async (item) => {
        if (!confirm(`¿Eliminar el estatus "${item.name}"?`)) return;
        try {
            await axios.delete(`/api/timedesk/employee-statuses/${item.id}`);
            setItems((prev) => prev.filter((r) => r.id !== item.id));
            notify.success("Estatus eliminado");
        } catch (err) {
            if (!handleAuthError(err)) {
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm mt-1">
                            Administre los estatus del empleado para el expediente de RH (ej. Entrevista, Capacitación, Activo, Baja).
                        </p>
                    </div>
                </div>
                {canManage && (
            ...
            <Dialog open={open} onOpenChange={(o) => { if (!saving) setOpen(o); if (!o) resetForm(); }}>
                <DialogContent className="sm:max-w-md border-border bg-background">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="es-desc">Descripción (opcional)</Label>
                            <Textarea
                                id="es-desc"
                                value={form.description}
                                onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                                placeholder="Breve descripción del estatus"
                                rows={2}
                        ...
                        {editing && (
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <Label htmlFor="es-active">Activo</Label>
                            </div>
                        )}
